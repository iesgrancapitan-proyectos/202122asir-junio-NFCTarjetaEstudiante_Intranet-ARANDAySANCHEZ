...
<body id="page">
    ...
    <?php $departamentos = $_POST['dep']; ?>
    <h1 id="unidad"> Consultando <em style="color:#3B3B3A"><?php echo $departamentos ?></em></h1>
        <div id="divcentral" class="container px-4 px-lg-5 d-flex h-100 align-items-center justify-content-center">
            <div class="d-flex justify-content-center">
                ...
                <form method='POST' action='realizar_baja_profesores.php'>
                <script>
                function cambio(){document.getElementById("servicio_name").value = document.getElementById("servicio").innerText}</script>
                <div id="divtabla"class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            $departamentos2 = $_POST['dep'];
                            $queryusuario = mysqli_query($conn,"SELECT idtarjeta FROM tabla_nfc_profesores where idtarjeta is NOT NULL and departamento = '$departamentos2'");
                            $nr = mysqli_num_rows($queryusuario); 
                            
                            if (($nr > 0)) { 
                            if (isset($_REQUEST['Acceder'])){
                                
                                $departamentos = $_POST['dep'];
                                include('conexionbd.php');
                                $query = "SELECT * from tabla_nfc_profesores where departamento = '$departamentos' order by usuario";
                                $query2 ="SELECT * from tabla_nfc_tarjeta";
                                $result = mysqli_query($conn,$query);
                                $result2= mysqli_query($conn,$query2);
                                $row2= mysqli_fetch_array($result2);
                                while($row= mysqli_fetch_array($result)){
                                    if ($row['idtarjeta'] != NULL){
                                        $user = $row['usuario'];
                                        $usuarios = explode("@", $user);
                                        echo "<tr> <td> <input type='radio' name='idtarjeta' required value=".$row['idtarjeta']."> ".$usuarios[0]."</option> </td>" . " <td> ". $row['idtarjeta'] ." </td></th>";
                                        ...
                                        echo " <td> ". "Sin Tarjeta Asignada" ." </td></tr>";}
                                }
                            }
                        }
                            else{
                                echo "<h1 class='form-control'style='color: red;'>No hay tarjetas en este departamento asignadas</h1>";
                                header( "refresh:3;url=baja_profesores.php" );
                            }
                ...
                <input type="submit" class="btn btn-primary" name= "enviar" value="Baja tarjeta"> 
