...
<body id="page" onload="acceso.nombredelacaja.focus()">
    ...
    $nie= $_SESSION['nie'];
    $query="Select * from alumnos WHERE NIE='$nie'";
    $result= mysqli_query($conn,$query);
    $nr= mysqli_num_rows($result);
    $row = mysqli_fetch_assoc($result);
    echo '<h2 id="letra2" class="mx-auto mt-2 mb-5">'.'Nombre: ' .$row['Nombre'].  " "  .$row['Apellidos']. '</h2>';
    echo '<h2 id="letra2" class="mx-auto mt-2 mb-5">'.'Unidad: ' .$row['Unidad']. '</h2>';
    
    ?>
    <form method='post' action='modificar.php'>
        <h1>Introduzca nueva tarjeta</h1>
        <input type="text" name="nuevoid" required><br><br>
    ...
    <?php 
     ...
     if (isset($_REQUEST['Acceder']))
     {
       ...
       $nuevoid = $_REQUEST['nuevoid'];
       $modificar ="UPDATE tabla_nfc_estudiantes set idtarjeta='$nuevoid' where NIE= '$nieconsulta'";
       if(mysqli_query($conn,$modificar)){
        
        echo '<h4 id="letra2" class="mx-auto mt-2 mb-5">'.'Registro actualizado' .'</h4>';

        
    } else {
        echo '<h2 id="letra2" class="mx-auto mt-2 mb-5">' . 'Error: No se ejecutó' .$modificar. '</h2>';
        echo "ERROR: No se ejecuto $modificar. " . mysqli_error($conn);
    }
    // Cierra la conexion
    mysqli_close($conn);
    }
